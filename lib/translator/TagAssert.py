...
class TagAssert:
    ...
    @classmethod
    def create_file(cls, class_name):
        logger.info("Create path for %s"%class_name)
        file_path = class_name.replace('.', os.sep)
    ...
    @classmethod
    def create_dep_files(cls, dep_set, rb_loc):
        '''
    ...
    def generate(cls, rb_loc):
        ...
        for c in cls.classes_entry:
            ...
            cls.create_dep_files(dep_set, rb_loc)
            template_h.close()
            out_file.close()
